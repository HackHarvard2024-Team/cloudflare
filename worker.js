...
async function handleRequest(request) {
  ...
  if (!data || !data.polyline) {
      return new Response("Missing 'polyline' field in the request body", { status: 400 });
  }
  ...
  for (let i = 0; i < routePoints.length - 1; i++) {
      routeLines.push([routePoints[i], routePoints[i + 1]]);
  }

  const polygons = await fetch4Gons();
  const overlappingPolygons = polygons.filter(polygon => {
      return routeLines.some(line => lineIntersectsPolygon(line, polygon));
  });
